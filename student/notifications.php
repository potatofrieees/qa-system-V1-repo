...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header("Location: notifications.php" . ($redir_parts ? '?'.implode('&', $redir_parts) : ''));
    exit;
}
...
function u_notif_url(array $extra = []): string {
    global $filter, $per_page;
    $params = [];
...
<html lang="en">
<head>
    <title>Notifications — QA Portal</title>
...
<div class="main-content">
    ...
    <div class="page-body">
        ...
        <div style="display:flex;gap:8px;margin-bottom:20px;flex-wrap:wrap;">
            <a href="<?= u_notif_url(['filter'=>null,'page'=>null]) ?>" class="btn <?= $filter==='all' ? 'btn-primary':'btn-ghost' ?> btn-sm">
                All <?php if ($total_count): ?>(<?= $total_count ?>)<?php endif; ?>
            </a>
            <a href="<?= u_notif_url(['filter'=>'unread','page'=>null]) ?>" class="btn <?= $filter==='unread' ? 'btn-primary':'btn-ghost' ?> btn-sm">
        ...
        <div class="notif-toolbar">
            <div class="notif-toolbar-actions">
                ...
                <form method="POST" style="display:inline;"
                      class="swal-confirm-form" data-title="Clear All Notifications?" data-text="All notifications will be permanently deleted. This cannot be undone." data-icon="warning" data-confirm="Yes, Clear All" data-cls="qa-btn-red">
            ...
            <div class="per-page-select">
                <span>Show:</span>
                <select onchange="window.location='<?= u_notif_url(['per_page'=>'__PP__','page'=>null]) ?>'.replace('__PP__', this.value)">
        ...
        <div class="card" style="overflow:hidden;">
            ...
            <div class="empty-state" style="padding:72px 24px;">
                ...
                <p style="color:var(--muted);">No notifications<?= $filter==='unread' ? ' — all caught up!' : '.' ?></p>
                <?php if ($filter === 'unread' && $total_count > 0): ?>
            ...
            <div class="notif-row <?= $unread ? 'unread' : '' ?>">
                ...
                <div class="notif-body">
                    <div class="notif-title <?= $unread ? '' : 'read' ?>"><?= htmlspecialchars($n['title'] ?? 'Notification') ?></div>
                    ...
                    <div class="notif-meta">
                        ...
                        <span style="font-size:.68rem;font-weight:700;color:<?= $pc ?>;text-transform:uppercase;"><?= $n['priority'] ?></span>
                        <?php endif; ?>
                    </div>
                    <div class="notif-actions">
                        ...
                        <?php endif; ?>
                        <form method="POST" style="display:inline;"
                              class="swal-confirm-form" data-title="Delete Notification?" data-text="This notification will be permanently removed." data-icon="warning" data-confirm="Yes, Delete" data-cls="qa-btn-red">
        ...
        <div class="pagination">
            <?php if ($page > 1): ?>
            <a href="<?= u_notif_url(['page'=>$page-1]) ?>" class="btn btn-ghost btn-sm">← Prev</a>
            <?php endif; ?>
            <?php for ($p = max(1, $page-2); $p <= min($total_pages, $page+2); $p++): ?>
            <a href="<?= u_notif_url(['page'=>$p]) ?>" class="btn <?= $p===$page ? 'btn-primary' : 'btn-ghost' ?> btn-sm"><?= $p ?></a>
            <?php endfor; ?>
            <?php if ($page < $total_pages): ?>
            <a href="<?= u_notif_url(['page'=>$page+1]) ?>" class="btn btn-ghost btn-sm">Next →</a>
            <?php endif; ?>
